...
// Stocke les données des médias filtrés (1)
const filteredMedias = []; //(1)

async function filterMedia(sortDatasMedia) {
  const scrollingMenuSection_Id = document.querySelector('#scrolling-menu-id');

  scrollingMenuSection_Id.addEventListener('click', function (e) {
    ...
    sortDatasMedia.sort((a, b) => {
      ...
      if (sortMedias === 'Titre') {
        return a.title.localeCompare(b.title) - a.likes;
      }
    });
  });
}

// Ici nettoyer le conteneur avant d'appler la fonction

mediaFactory(onePhotographerData, sortDatasMedia);
