...
async function init() {
  ...
  // (7)
  //  const getNameId_Form = dataCible.find(
  //   (onePhotographer) => onePhotographer.id === getId
  // );
  // console.log(getNameId.name);

  // const headerModal_CLass = document.querySelector('.header-modal-class');
  const photographerNameClass = document.querySelector('.h2-photographer-name');
  const modelForm = photographerFactory(getNameId);
  const formDomModel = modelForm.displayFormNameDOMPhotographer();
